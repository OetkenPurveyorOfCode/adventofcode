...
int main(void) {
    ...
    int number_of_winning_boards = 0;
    for (int num_it = 0; num_it < numbers.len; num_it++) {
        for (int board_it = 0; board_it < boards.len; board_it++) {
            if (boards.data[board_it].won == false) {
                if (handle_number(numbers.data[num_it], &boards.data[board_it])){
                    number_of_winning_boards += 1;
                    if (number_of_winning_boards == boards.len) {
                        last_winning_board = boards.data[board_it];
                        last_winning_number = numbers.data[num_it];
                    }
                }
            }
        }
...
score:
    ...
    int final_score = calculate_score(last_winning_board, last_winning_number);
